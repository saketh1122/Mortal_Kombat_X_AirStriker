...
def init_model(output_path, player_model, player_alg, args, env, logger):
    ...
    if player_alg == 'ppo2':
        if player_model == '':
            batch_size = (128 * args.num_env) // 4
            com_print(f"[init_model] Initializing NEW PPO model with batch_size: {batch_size}")
            model = PPO(
                policy=args.nn,
                env=env,
            ...
            checkpoint_path = os.path.expanduser(player_model)
            com_print(f"[init_model] Loading PPO model from checkpoint: {checkpoint_path}")
            model = PPO.load(checkpoint_path, env=env, device="cuda")
    elif player_alg == 'a2c':
        if player_model == '':
            com_print("[init_model] Initializing NEW A2C model")
            model = A2C(
                policy=args.nn,
                env=env,
                policy_kwargs=policy_kwargs,
            ...
            checkpoint_path = os.path.expanduser(player_model)
            com_print(f"[init_model] Loading A2C model from checkpoint: {checkpoint_path}")
            model = A2C.load(checkpoint_path, env=env)
    else:
        raise ValueError(f"Unsupported algorithm: {player_alg}")

    # Optionally set logger if needed
